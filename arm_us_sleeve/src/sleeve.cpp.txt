...
// Global objects
ros::NodeHandle n;
std_msgs::Float32MultiArray accel_msg;
...
void setup()
{
    ...
    accel1.dmpInitialize();
    accel1.setXGyroOffset(0);
    accel1.setYGyroOffset(0);
    accel1.setZGyroOffset(0);
    accel1.setZAccelOffset(1688); // 1688 factory default for my test chip

    // accel1.CalibrateAccel(6);
    accel1.CalibrateGyro(6);
    accel1.setDMPEnabled(true);
    packetSize = accel1.dmpGetFIFOPacketSize();
...
void loop()
{
    ...
    while (fifoCount < packetSize) 
        fifoCount = accel1.getFIFOCount();

    uint8_t data[packetSize];
    float euler[3];
    Quaternion q;
    accel1.dmpGetQuaternion(&q, data);


    int a = int(euler[0] * 180/M_PI);
    char buffer[10];
    itoa(a, buffer, 10);
    n.logerror(buffer);

    accel1.dmpGetEuler(euler, &q);
    
    std_msgs::Float32MultiArray msg;
    msg.data_length = 3;

    float angle_values[3];

    angle_values[0] = euler[0] * 180/M_PI;
    angle_values[1] = euler[1] * 180/M_PI;
    angle_values[2] = euler[2] * 180/M_PI;

    msg.data = angle_values;

    pub_accel_wrist.publish(&msg);
    delay(10);
